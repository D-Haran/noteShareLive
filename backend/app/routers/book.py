...
def get_books(current_user: int = Depends(oauth2.get_current_user), db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    results = db.query(models.Book.id, models.Book.title, models.Book.description, models.Book.owner_id, models.Book.library_id, 
                            models.Book.file, models.Book.thumbnail, models.Book.created_at,
...
def get_books(current_user: int = Depends(oauth2.get_current_user), db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    main_query = db.query(models.Book.id, models.Book.title, models.Book.description, models.Book.owner_id, models.Book.library_id, 
                            models.Book.file, models.Book.thumbnail, models.Book.created_at,
                            models.User.username.label('owner'),
                            models.Library.title.label('library'),
                            func.count(models.Comment.id).label('comments'),
...
@router.get("/user-public-books/{id}")
def get_books(id: int, current_user: int = Depends(oauth2.get_current_user), db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    main_query = db.query(models.Book.id, models.Book.title, models.Book.description, models.Book.owner_id, models.Book.library_id, 
                            ...
                            models.User, models.Book.owner_id == models.User.id, isouter=True).filter(
                            models.Book.library_id.in_(sub_query)).group_by(
                            models.Book.id, models.BookVote.user_id, models.BookVote.dir, models.Library.title, models.User.username).filter(models.Book.owner_id==id)
    results = main_query.filter(models.Book.title.contains(search)).limit(limit).offset(skip).all()
    return results

@router.get("/user-public-books-default/{id}")
def get_books(id: int, db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    user = db.query(models.User).filter(models.User.id==id).first()
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"User with id: {id} does not exist")
...
def get_books(current_user: int = Depends(oauth2.get_current_user), db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    main_query = db.query(models.Book.id, models.Book.title, models.Book.description, models.Book.owner_id, models.Book.library_id, 
                            ...
                            models.Book.id, models.BookVote.user_id, models.BookVote.dir, models.Library.title, models.User.username).filter(models.BookVote.dir==1)
    results = main_query.filter(models.Book.title.contains(search)).limit(limit).offset(skip).all()
    return results
...
def get_books_from_library(id: int, current_user: int = Depends(oauth2.get_current_user), db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    library_check = db.query(models.Library).filter(models.Library.id==id).first()
    if not library_check:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Library with id: {id} does not exist")
    query = db.query(models.Book.id, models.Book.title, models.Book.description, models.Book.owner_id, models.Book.library_id, 
                            models.Book.file, models.Book.thumbnail, models.Book.created_at,
                            models.User.username.label('owner'),
                            models.Library.title.label('library'),
                            func.count(models.Comment.id).label('comments'),
...
@router.get("/from-library-default/{id}")
