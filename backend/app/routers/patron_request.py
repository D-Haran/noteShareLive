...
def patron_request(
    ...
    if patron_request.admin_level == "author":
        ...
        # check if the patron request has already been made
        request = db.query(models.PatronRequest).filter(
            models.PatronRequest.requester_id==current_user.id,
            models.PatronRequest.requestee_id==librarian_id, 
            models.PatronRequest.library_id==patron_request.library_id,
            models.PatronRequest.admin_level==patron_request.admin_level).first()
        if request:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, 
                                detail=f"patron request already exists")
...
def get_patron_requests(
    ...
    query = db.query((models.PatronRequest.id).label("request_id"),
                        ...
                        (models.PatronRequest.admin_level).label("requested_admin_level"),
...
@router.get("/check-request/{id}")
def check_patron_request(id: int,
                        db: Session = Depends(get_db),
                        current_user: int = Depends(oauth2.get_current_user)):
        # check if the patron request has already been made
        request = db.query(models.PatronRequest).filter(
            models.PatronRequest.requester_id==current_user.id,
            models.PatronRequest.library_id==id,
            models.PatronRequest.admin_level=="author").first()
        if request:
            return {"result": True}
        return {"result": False}
