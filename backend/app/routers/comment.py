...
def get_comments_on_book(id: int,
    ...
    comment_query = db.query(models.Comment.id, models.Comment.content, models.Comment.created_at, models.Comment.user_id, models.Comment.book_id,
...
def get_comments_on_book(id: int,
    ...
    # first get the library_id of the current book
    library_id = db.query(models.Book.library_id).filter(models.Book.id==id).first()[0]
    # next get user_id's of all authors and librarian of the current book
    mod_user_ids = db.query(models.Patron.user_id).filter(
                            models.Patron.library_id==library_id,
                            or_(models.Patron.admin_level == "author", models.Patron.admin_level =="librarian"))
    comment_query = db.query(models.Comment.id, models.Comment.content, models.Comment.created_at, models.Comment.user_id, models.Comment.book_id,
                            models.User.username.label('owner'),
                            models.Patron.admin_level,
                            case((and_((models.User.id==current_user.id), (models.User.id.in_(mod_user_ids))), True), else_=False).label('deletable'),
