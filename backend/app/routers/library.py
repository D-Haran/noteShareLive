...
def get_libraries(
    ...
    result_query = db.query(models.Library.id, models.Library.title,
                        models.Library.description, models.Library.public,
                        models.Library.banner, models.Library.created_at, models.Library.owner_id,
...
def get_public_libraries(
    ...
    user_level = aliased(models.Patron)
    query = db.query(models.Library.id, models.Library.title,
                        models.Library.description, models.Library.public,
                        models.Library.banner, models.Library.created_at, models.Library.owner_id,
                        models.User.username.label('owner'), user_level.admin_level,
                        func.count(distinct(models.Patron.user_id)).label("patrons"),
                        func.count(distinct(models.Book.id)).label("books")).join(
                        models.User, models.User.id == models.Library.owner_id, isouter=True).join(
                        models.Patron, models.Patron.library_id == models.Library.id, isouter=True).join(
                        models.Book, models.Book.library_id == models.Library.id, isouter=True).join(
                        user_level, and_(models.Library.id==user_level.library_id, user_level.user_id == current_user.id), isouter=True).filter(
                        models.Library.id.in_(sub_query)).group_by(models.Library.id, models.User.username, user_level.admin_level).order_by(
                            desc("patrons"))
...
def get_public_libraries(
    ...
    query = db.query(models.Library,
                        ...
                        models.Book, models.Book.library_id == models.Library.id, isouter=True).join(
                        user_level, and_(models.Library.id==user_level.library_id, user_level.user_id == current_user.id), isouter=True).filter(
                        models.Library.id.in_(sub_query)).group_by(models.Library.id, models.User.username, user_level.admin_level)
    result = query.filter(models.Library.id==id).first()
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"library with id: {id} does not exist")
    return result
...
def get_my_libraries(
    ...
    query = db.query(models.Library.id, models.Library.title,
                        ...
                        func.count(distinct(models.Book.id)).label("books")).join(
                        models.Patron, models.Patron.library_id == models.Library.id, isouter=True).join(
                        models.Book, models.Book.library_id == models.Library.id, isouter=True).filter(
                        models.Library.id.in_(sub_query)).group_by(models.Library.id)
    result = query.all()
    return result
...
def delete_library(id: int,
    ...
    if library.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail=f"Not authorized to perform requested action")
    if library.banner != str(settings.default_banner):
